...
function drawControlPoints(path) {
  ...
  const DUP_R = ANCHOR_R_PX * px * 0.9; // ~1 anchor radius in world units

  if (anchors.length > 1) {
    const w0 = localToWorld(path, anchors[0].x, anchors[0].y);

    anchors = anchors.filter((a, i) => {
      if (i === 0) return true;
      const w = localToWorld(path, a.x, a.y);
      return Math.hypot(w.x - w0.x, w.y - w0.y) > DUP_R;
    });
  }
  ...
  handles.forEach((handle, qIndex) => {
    ...
    const HIDE_R = HANDLE_R_PX * px * 0.9;
    if (Math.hypot(wCtrl.x - wAnchor.x, wCtrl.y - wAnchor.y) <= HIDE_R) {
      return; // handle dot would sit on the anchor
    }
